...
describe('multiple requests with custom callbacks', function () {
	it('get it done', function (done) {
		...
		const	exchangeName	= 'test_multi_request',
			intercom1	= new Intercom(require(intercomConfigFile)),
			intercom2	= new Intercom(require(intercomConfigFile)),
			tasks	= [];
		...
		tasks.push(function (cb) {
			...
			syncServer.handleHttpReq = function (req, res) {
				...
				// Run different commands depending on request url
				if (req.url === '/1') {
					syncServer.options.dataDumpCmd = {'command': 'echo', 'args': ['-n', '1']};
				} else if (req.url === '/2') {
					syncServer.options.dataDumpCmd = {'command': 'echo', 'args': ['-n', '2']};
				} else {
					syncServer.options.dataDumpCmd = {'command': 'echo', 'args': ['-n', 'error']};
				}
				...
				syncClient.handleMsg_original(message, ack, function (err, res) {
					...
					res.on('end', function () {
						...
						syncClient.handleMsg_original(message, function () {}, function (err, res) {
							...
							res.on('end', function () {
								assert.deepEqual(syncData.toString(), '2');
								cb();
							});
